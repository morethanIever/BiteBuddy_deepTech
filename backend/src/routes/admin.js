...
router.get('/stats', requireAuth, (req, res) => {
  ...
  const recentScans = db.prepare(`
    SELECT s.id,s.result,s.score,s.salmonella,s.ecoli,s.staph,s.created_at,
...
router.get('/devices', requireAuth, (req, res) => {
  const devices = db.prepare(
    'SELECT dk.*, r.name AS restaurant_name FROM device_keys dk LEFT JOIN restaurants r ON r.id=dk.restaurant_id ORDER BY dk.id DESC'
  ).all();
